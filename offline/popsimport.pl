...
$csv->read_header();
while ($csv->get_row()) {
	my ($address, $name, $location, @ps) = $csv->extract(
		"Building Address","Building Name","Building Location","Public Space 1","PS 1 Size","Public Space 2","PS 2 Size","Public Space 3","PS 3 Size","Public Space 4","PS 4 Size","Public Space 5","PS 5 Size");
	$name ||= "";
	...
	my $tries = 0;
	while (!$success) {
		my $response = $ua->get("http://maps.googleapis.com/maps/api/geocode/json?sensor=false&address=" . $address_param);
		if ($response->is_success) {
			...
			if ($geodata->{'status'} eq 'OVER_QUERY_LIMIT') {
				...
				$success = 1;
				say $file to_json({'name'=>$name,'address'=>$address,'location'=>$location,"spaces"=>\%spaces, "geodata" => $geodata}, {pretty=>1});
			} else {
